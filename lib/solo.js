...
import { track, unique, hashId, statsOff } from "./stats.js";
import { bankAdd, bankAnswer } from "./bank.js";

const DAY_TTL = 9 * 86400, CH_TTL = 30 * 86400; // Daily records last 9 days so return visits can be measured over a week
const DAILY_N = 8;
...
async function loadLadder(kind, ref, openaiKey) {
  ...
  try {
    ...
    await bankAdd(topic, questions, "daily");
...
async function boardView(kind, ref, device) {
  ...
  // Test runs play the real Daily; their players are stored but only ever shown to themselves.
  const rows = Object.entries(h).map(([key, v]) => ({ ...v, key, isMe: key === device })).filter((r) => !r.test || r.isMe)
...
// Daily return visits: did this device also play yesterday, or in the week before? Counted once, when today's game begins.
async function dailyReturn(ref, device) {
  const prev = (d) => new Date(Date.parse(ref + "T00:00:00Z") - d * 86400000).toISOString().slice(0, 10);
  const seen = await Promise.all(Array.from({ length: 8 }, (_, i) => store.get(K.player("daily", prev(i + 1), device)).then((v) => v != null).catch(() => false)));
  let streak = 1; for (const x of seen) { if (!x) break; streak++; }
  return { ret: seen[0] ? "yesterday" : seen.slice(0, 7).some(Boolean) ? "week" : "new", streak: streak >= 7 ? "7+" : streak >= 3 ? "3-6" : String(streak) };
}

export async function soloState({ kind, ref, device, token, name, level, begin, peek, src }, openaiKey) {
  ...
  if (!p) {
    ...
    if (kind === "daily") { const r = await dailyReturn(ref, device); await Promise.all([track("daily_start", { ...r, src }), unique("daily", device), unique("players", device)]); }
    else await Promise.all([track("challenge_start", { src }), unique("players", device)]);
...
export async function soloAnswer({ kind, ref, device, token, q, text, double }, jevKey, ip = "unknown") {
  ...
  const n0 = await store.hincrby(`rate:solo:${hour}`, hashId(ip), 1);
  ...
  await bankAnswer(question, ans, !statsOff());
  if (double) p.doubleUsed = true;
  if (p.cur >= n) { p.finishedAt = now(); await track(kind === "daily" ? "daily_finish" : "challenge_finish"); }
  await setJson(key, p, { ex: ttlFor(kind) });
  await hsetJson(K.board(kind, ref), device, { name: p.name, level: p.level, total: p.total, done: p.cur, finished: p.cur >= n, grid: p.answers.map((a) => (a.good ? 2 : a.points > 0 ? 1 : 0)), ...(ip === "test-runner" ? { test: true } : {}) });
