...
async function buildLadder(topic, n, openaiKey, avoid = []) {
  const [{ questions }, unsafe] = await Promise.all([generateQuestions(topic, n, openaiKey, avoid), isUnsafe(topic, openaiKey)]);
...
export async function prepareRoom({ code: c, playerId, token }, openaiKey, ip = "unknown") {
  ...
  if (cur && cur.sig === sig && (cur.questions || cur.rejected || (cur.pending && now() - cur.at < 45000))) return { ok: true, state: cur.questions ? "ready" : cur.rejected ? "rejected" : "pending" };
...
export async function startRoom({ code: c, playerId, token }, openaiKey, ip = "unknown") {
  ...
  try {
    ...
    for (let w = 0; prep?.sig === sig && prep.pending && w < 60; w++) { await new Promise((r) => setTimeout(r, 500)); prep = await getJson(K.prep(c)); }
    ...
    Object.assign(fresh, { questions, status: "playing", round: { index: 0, startedAt: t0, endsAt: roundEndsAt(fresh, t0) }, games: (fresh.games || 0) + 1, startedAt: t0, finishedAt: null });
...
export function scoreAnswer({ result, question, numQuestions, elapsedMs, limitMs, streak, double }) {
  ...
  if (coverage === 0 || (result.flags?.wrong && coverage < 0.67)) return zero({ wrong: true });
...
export async function submitAnswer({ code: c, playerId, token, q, text, double }, apiKey) {
  ...
  if (text.length < 3) throw new GameError("Write a little more first.");
...
export async function playAgain({ code: c, playerId, token, topic, numQuestions, mode, secondsPerQ }) {
  ...
  Object.assign(room, { status: "lobby", questions: [], round: null, error: null });
...
export async function getState(body, apiKey) {
  ...
  if (!body._again && room.mode !== "race") {
    ...
    if (room.status === "playing" && room.round) {
      ...
      if ((room.mode === "timed" && t > room.round.revealedAt + REVEAL_AUTO_MS) || allReady) {
        const fresh = await loadRoom(c);
  ...
  const out = {
    ...
    countdownMs: COUNTDOWN_MS, revealAutoMs: room.mode === "timed" ? REVEAL_AUTO_MS : 0, reactions: REACTIONS,
  };
  ...
  if (live && room.mode !== "race") {
    ...
    out.round = { index: roundIdx, startedAt: room.round.startedAt, endsAt: room.round.endsAt, revealedAt: room.round.revealedAt || null };
