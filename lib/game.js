...
// ---------- lobby ----------
export async function createRoom({ name, topic, mode, secondsPerQ, numQuestions, level, device }, ip = "unknown") {
...
export async function joinRoom({ code: c, name, level, device }, ip = "unknown") {
  ...
  if (mine) { await touch(c, mine.id); return { code: c, playerId: mine.id, token: mine.token, rejoined: true }; }
...
async function buildLadder(topic, n, openaiKey, avoid = [], onStage) {
  const [{ questions, ms, steps, rewritten }, unsafe] = await Promise.all([generateQuestions(topic, n, openaiKey, avoid, onStage), isUnsafe(topic, openaiKey)]);
...
export async function startRoom({ code: c, playerId, token }, openaiKey, ip = "unknown") {
  ...
  try {
    ...
    fresh.error = err instanceof TopicRejected ? REJECTED_MSG : unavailable ? "New games can't start right now. Please try again in a few minutes. Today's Daily Clash is still open." : "Could not build questions for that topic. Try again, or try a different topic.";
