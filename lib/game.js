...
async function limit(bucket, ip, max, windowSec = 3600) {
  if (process.env.RATE_LIMIT_OFF) return;
...
export async function startRoom({ code: c, playerId, token }, openaiKey, ip = "unknown") {
  ...
  try {
    const [{ questions }, unsafe] = await Promise.all([generateQuestions(room.topic, room.numQuestions, openaiKey), isUnsafe(room.topic, openaiKey)]);
    ...
    const fresh = await loadRoom(c);
    fresh.status = "lobby";
    fresh.error = err instanceof TopicRejected ? "FactClash keeps it friendly and factual. Try a different topic." : "Could not build questions for that topic. Try again, or try a different topic.";
