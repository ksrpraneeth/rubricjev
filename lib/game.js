...
import { track, unique, hashId, statsDay, isTestTraffic, statsOff } from "./stats.js";
import { bankAdd, bankAnswer, bankSkip } from "./bank.js";
...
const CHILL_PATIENCE_MS = 90000; // chill: once anyone has answered and 90s have passed, the last call starts anyway
...
// Per-IP hourly limits. Many phones can share one public IP (carrier NAT, schools, offices), so the cheap
// actions are generous and the ones that spend on question writing are moderate.
async function limit(bucket, ip, max, windowSec = 3600) {
  ...
  const n = await store.hincrby(k, hashId(ip), 1); // a one-way hash, never the raw IP, kept for an hour
  if (n === 1) await store.expire(k, windowSec);
  if (n > max) throw new GameError("Slow down a little and try again soon.", 429);
}
export const limitJourney = (ip) => limit("journey", ip, 900);
const sizeBucket = (n) => (n <= 1 ? "1" : n <= 3 ? "2-3" : n <= 6 ? "4-6" : "7-12");
...
// ---------- lobby ----------
export async function createRoom({ name, topic, mode, secondsPerQ, numQuestions, level, device, src }, ip = "unknown") {
  await limit("create", ip, 150);
  name = cleanName(name);
  ...
  await saveRoom(room);
  device = String(device || "").replace(/[^a-z0-9]/gi, "").slice(0, 32);
  await hsetJson(K.players(c), hostId, { id: hostId, name, avatar: 0, level: cleanLevel(level), token, joinedAt: now(), ...(device.length >= 12 ? { device } : {}) });
  await touch(c, hostId);
  await expireAll(K.players(c), K.live(c));
  await Promise.all([track("room_created", { mode, src }), unique("players", device), unique("hosts", device)]);
  return { code: c, playerId: hostId, token };
}

export async function joinRoom({ code: c, name, level, device, via, src }, ip = "unknown") {
  await limit("join", ip, 1500); // cheap; generous because many phones share one carrier or school IP
  ...
  // The same phone joining again (a retry after a lost reply, or a lost session) gets its own seat back.
  device = String(device || "").replace(/[^a-z0-9]/gi, "").slice(0, 32);
  const mine = device.length >= 12 && Object.values(players).find((p) => p.device === device);
  if (mine) { await touch(c, mine.id); await track("rejoin"); return { code: c, playerId: mine.id, token: mine.token, rejoined: true }; }
  ...
  await hsetJson(K.players(c), pid, { id: pid, name, avatar, level: cleanLevel(level), token, joinedAt: now(), ...(device.length >= 12 ? { device } : {}) });
  await touch(c, pid);
  await expireAll(K.players(c), K.live(c));
  await logEvent(c, "join", `${name} joined`, { playerId: pid, name });
  await Promise.all([track("join", { via, src, status: room.status === "lobby" ? "lobby" : "late" }), unique("players", device)]);
...
export async function endGame({ code: c, playerId, token }) {
  ...
  await track("game_finished", { mode: room.mode, how: "ended" });
  return { ok: true };
}
...
// Records how far question writing has got, so the lobby and the "Setting the stage" screen can say so.
const stageWriter = (c, sig, at) => async (stage) => {
  const cur = await getJson(K.prep(c));
  if (cur && (cur.sig !== sig || !cur.pending)) return;
  await setJson(K.prep(c), { sig, pending: true, at, stage }, { ex: DAY }); notify(c);
};
// Spend guard: at most GEN_DAILY_CAP new question sets a day (about $0.034 each), so a spike or abuse
// can never run up an unbounded bill. Test runs do not count. The Daily Clash is built separately by the cron.
export class Paused extends Error {}
const GEN_CAP = Number(process.env.GEN_DAILY_CAP || 2000);
async function buildLadder(topic, n, openaiKey, avoid = [], onStage) {
  const budgetKey = `gen:${statsDay()}`, counted = !isTestTraffic();
  if (counted && Number(await store.hget(budgetKey, "n") || 0) >= GEN_CAP) { await track("ladder_failed", { why: "cap" }); throw new Paused("daily cap"); }
  let result;
  try { result = await Promise.all([generateQuestions(topic, n, openaiKey, avoid, onStage), isUnsafe(topic, openaiKey)]); }
  catch (err) {
    const why = err instanceof TopicRejected ? "topic" : /OpenAI (429|5\d\d)|insufficient_quota|fetch failed|aborted/i.test(String(err?.message || "")) ? "unavailable" : "error";
    await track("ladder_failed", { why });
    throw err;
  }
  const [{ questions, ms, steps, rewritten }, unsafe] = result;
  if (counted) { await store.hincrby(budgetKey, "n", 1); await store.expire(budgetKey, 3 * DAY); }
  await track("ladder_built", { n, rewritten: rewritten ? "yes" : "no" });
  await bankAdd(topic, questions, "room");
  if (unsafe) { await track("ladder_failed", { why: "moderation" }); throw new TopicRejected("moderation"); }
...
export async function prepareRoom({ code: c, playerId, token }, openaiKey, ip = "unknown") {
  ...
  const at = now();
  await setJson(K.prep(c), { sig, pending: true, at, stage: "write" }, { ex: DAY }); notify(c);
  try {
    const questions = await buildLadder(room.topic, room.numQuestions, openaiKey, room.asked || [], stageWriter(c, sig, at));
    const latest = await getJson(K.prep(c));
    if (latest?.sig !== sig) return { ok: true, state: "stale" };
    await setJson(K.prep(c), { sig, questions, at: now() }, { ex: DAY }); notify(c);
    return { ok: true, state: "ready" };
  } catch (err) {
    const latest = await getJson(K.prep(c));
    if (latest?.sig !== sig) return { ok: true, state: "stale" };
    if (err instanceof TopicRejected) { await setJson(K.prep(c), { sig, rejected: true, at: now() }, { ex: DAY }); notify(c); return { ok: true, state: "rejected" }; }
...
export async function startRoom({ code: c, playerId, token }, openaiKey, ip = "unknown") {
  const room = await loadRoom(c); await auth(c, playerId, token);
  await limit("start", ip, 150);
  await limit("startroom", c, 20);
  if (room.hostId !== playerId) throw new GameError("Only the host can start.", 403);
  // A repeated Start (a retry after a lost reply) is fine once the game is under way.
  if (room.status === "generating" || room.status === "playing") return { ok: true, already: true };
  ...
  try {
    ...
    const questions = prep?.sig === sig && prep.questions ? prep.questions : await buildLadder(room.topic, room.numQuestions, openaiKey, room.asked || [], stageWriter(c, sig, now()));
    ...
    notify(c);
    const count = Object.keys(await hgetallJson(K.players(c))).length;
    await Promise.all([track("game_started", { mode: fresh.mode, size: sizeBucket(count), prep: prep?.sig === sig && prep.questions ? "ready" : "waited", rematch: fresh.games > 1 ? "yes" : "no" }), track("game_players", {}, count)]);
    return { ok: true };
  } catch (err) {
    if (!(err instanceof TopicRejected)) console.error("generate", err);
    const fresh = await loadRoom(c);
    fresh.status = "lobby";
    // When the question writer itself is unavailable (out of credit, rate limited, down), the topic is not the problem.
    const unavailable = /OpenAI (429|5\d\d)|insufficient_quota|fetch failed|aborted/i.test(String(err?.message || ""));
    fresh.error = err instanceof TopicRejected ? REJECTED_MSG : err instanceof Paused ? "So many games today that new ones are taking a short break. Please try again later. Today's Daily Clash is still open." : unavailable ? "New games can't start right now. Please try again in a few minutes. Today's Daily Clash is still open." : "Could not build questions for that topic. Try again, or try a different topic.";
...
async function gradeOne(room, qIndex, pid, ans, apiKey) {
  ...
  await bankAnswer(q, graded, !ans.ns);
  return graded;
}
...
async function gradeRound(room, qIndex, apiKey) {
  ...
  await Promise.all(Object.keys(players).map(async (pid) => {
    ...
    if (!a) {
      await bankSkip(room.questions[qIndex]);
...
export async function submitAnswer({ code: c, playerId, token, q, text, double }, apiKey) {
  ...
  if (room.mode === "race") {
    ...
    const ans = { text, submittedAt: now(), ms: now() - startedAt, double: !!double, ...(statsOff() ? { ns: 1 } : {}) };
    ...
    if (Object.keys(players).every((pid) => Number(prog[pid] || 0) >= room.questions.length || !isActive(seen, pid, t))) {
      ...
      await track("game_finished", { mode: "race", how: "complete" });
  ...
  if (double) await useDouble(c, playerId, q);
  const ans = { text, submittedAt: now(), ms: Math.max(0, now() - room.round.startedAt - (room.mode === "timed" ? COUNTDOWN_MS : 0)), double: !!double, ...(statsOff() ? { ns: 1 } : {}) };
  await hsetJson(K.answers(c, q), playerId, ans);
...
async function advance(room) {
  ...
  if (nextIndex >= room.questions.length) { room.status = "finished"; room.finishedAt = now(); track("game_finished", { mode: room.mode, how: "complete" }); }
  else { const t0 = now(); room.status = "playing"; room.round = { index: nextIndex, startedAt: t0, endsAt: roundEndsAt(room, t0) }; }
  await saveRoom(room);
}

// The host either reveals a chill round early (force) or moves on from a reveal. Both name the round they
// are for, so a repeated or late request for a round that has already moved on changes nothing.
export async function nextRound({ code: c, playerId, token, q, force }) {
  const room = await loadRoom(c); await auth(c, playerId, token);
  if (room.hostId !== playerId) throw new GameError("Only the host can advance.", 403);
  const stale = q != null && Number(q) !== room.round?.index;
  if (stale) return { ok: true, stale: true };
  if (room.status === "playing" && room.mode === "untimed" && (force || q == null)) { room.round.endsAt = now() - GRACE_MS - 1; await saveRoom(room); return { ok: true, forced: true }; }
  if (force || room.status !== "reveal") { if (q != null) return { ok: true, stale: true }; throw new GameError("Nothing to advance."); }
  await advance(room);
  return { ok: true };
}
...
export async function playAgain({ code: c, playerId, token, topic, numQuestions, mode, secondsPerQ }) {
  ...
  // Only a finished game can be restarted; a repeated request after the rematch began changes nothing.
  if (room.status === "lobby") return { ok: true, already: true };
  if (room.status !== "finished") throw new GameError("This game is still going.");
  Object.assign(room, { status: "lobby", questions: [], round: null, error: null, raceEndsAt: null });
  const topicChanged = !!topic && cleanTopic(topic) !== room.topic;
  if (topicChanged) { const t = cleanTopic(topic); checkTopic(t); room.topic = t; }
  ...
  await track("rematch", { topic: topicChanged ? "new" : "same" });
  return { ok: true };
}
...
export async function getState(body, apiKey) {
  ...
  if (!body._again && room.mode === "race" && room.status === "playing" && room.raceEndsAt && t > room.raceEndsAt) {
    const fresh = await loadRoom(c);
    if (fresh.status === "playing") { fresh.status = "finished"; fresh.finishedAt = t; await saveRoom(fresh); track("game_finished", { mode: "race", how: "deadline" }); notify(c); return getState({ ...body, _again: true }, apiKey); }
  }
  ...
  if (!body._again && room.mode !== "race") {
    ...
    if (room.status === "playing" && room.round) {
      ...
      else if (room.mode === "untimed" && !room.round.endsAt && Object.keys(currentAnswers).length && t > room.round.startedAt + CHILL_PATIENCE_MS) {
        // Idle players never send anything, so time alone starts the last call once someone has locked in.
        const fresh = await loadRoom(c);
        if (fresh.status === "playing" && fresh.round.index === roundIdx && !fresh.round.endsAt) { fresh.round.endsAt = t + LAST_CALL_MS; fresh.round.lastCall = true; await saveRoom(fresh); changed = true; }
      }
  ...
  list.forEach((p) => (p.rank = 1 + list.filter((x) => x.score > p.score).length)); // ties share a rank: 1, 1, 3
  ...
  const out = {
    ...
    raceEndsAt: room.mode === "race" ? room.raceEndsAt || null : null, chillPatienceMs: CHILL_PATIENCE_MS,
  };
  if (room.status === "generating") out.generatingFor = t - (room.generatingSince || t);
  if (room.status === "generating" || (room.status === "lobby" && room.hostId === me.id)) {
    const prep = await getJson(K.prep(c)), mine = prep?.sig === prepSig(room);
    if (room.status === "lobby") {
      out.prep = mine ? (prep.questions ? "ready" : prep.rejected ? "rejected" : "pending") : null;
      if (out.prep === "rejected") out.error = REJECTED_MSG;
    }
    out.prepStage = mine && prep.pending ? prep.stage || "write" : null;
  }
