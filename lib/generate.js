...
${SAFETY}`;
  ...
  const bad = new Map(verdicts.filter((v) => !v.ok).map((v) => [v.i, v.problems]));
  ...
  const order = live.map((q) => q.index).sort((a, b) => ((rate.get(a) ?? (1 + (9 * a) / (M - 1))) - (rate.get(b) ?? (1 + (9 * b) / (M - 1)))) || a - b);
  const P = order.length;
  const targets = Array.from({ length: n }, (_, k) => Math.round((k * (P - 1)) / Math.max(1, n - 1)));
  const used = new Set(), pickAt = new Array(n).fill(null);
  const nearest = (t, ok) => { for (let d = 0; d < P; d++) for (const p of [t - d, t + d]) if (p >= 0 && p < P && !used.has(p) && ok(order[p])) return p; return -1; };
  for (const k of [0, n - 1, ...Array.from({ length: n - 2 }, (_, j) => j + 1)]) { const p = nearest(targets[k], (x) => !bad.has(x)); if (p >= 0) { used.add(p); pickAt[k] = p; } }
  const holes = pickAt.map((v, k) => (v == null ? k : -1)).filter((k) => k >= 0);
  let rewritten = 0;
  if (holes.length) {
    const fix = holes.map((k) => { const p = nearest(targets[k], (x) => bad.has(x)); if (p >= 0) used.add(p); return { k, p, i: order[p] }; }).filter((h) => h.p >= 0);
    const others = pickAt.filter((p) => p != null).map((p) => drafts[order[p]]);
    // One call writes every replacement together, so they cover different facts from each other too.
    const fixed = fix.length ? await chat([
      { role: "system", content: system },
      { role: "user", content: `Topic: ${topic}\nOther questions in this game (each replacement must cover a different fact from all of them and from each other):\n${others.map((q) => `- ${q.prompt}`).join("\n")}\n\nQuestions to write (${fix.length}):\n${fix.map(({ i }) => `Question ${i + 1}/${M}: ${angles[i].angle} (harder because: ${angles[i].why_harder})\n  A previous draft was rejected because ${bad.get(i).join("; and ")}. Draft to avoid: "${drafts[i].prompt}"`).join("\n")}\nFix each one. If an angle itself caused the problem, pick a different, very well documented fact at the same difficulty. Never put any part of the answer in the question.` },
    ], "ladder_chunk", expandSchema, apiKey).then((r) => fix.map((h, j) => r.parsed.questions[j] ? { ...drafts[h.i], ...shape(r.parsed.questions[j]), rewritten: true } : null)).catch(() => fix.map(() => null)) : [];
    steps.fix = Date.now() - t0;
    const again = new Set((await review(fixed.filter(Boolean), topic, apiKey, [...others, ...fixed.filter(Boolean)])).filter((v) => !v.ok).map((v) => v.i));
    steps.recheck = Date.now() - t0;
    fix.forEach((h, j) => { const q = fixed[j]; if (q) { drafts[h.i] = q; rewritten++; } pickAt[h.k] = h.p; if (q && again.has(h.i)) steps.unfixed = (steps.unfixed || 0) + 1; });
  }
  const chosen = pickAt.filter((p) => p != null).map((p) => drafts[order[p]]);
  ...
  steps.flagged = bad.size; steps.drafts = M; steps.rated = chosen.map((q) => rate.get(q.index) ?? "?").join(",");
...
const reviewSchema = { type: "object", additionalProperties: false, properties: { verdicts: { type: "array", items: { type: "object", additionalProperties: false, properties: { index: { type: "integer" }, ok: { type: "boolean" }, problem: { type: "string" }, difficulty: { type: "integer" } }, required: ["index", "ok", "problem", "difficulty"] } } }, required: ["verdicts"] };
...
- duplicate: it asks for the same fact as an earlier question in this list.
...
Also rate every question's "difficulty" from 1 to 10: how hard it is for a typical adult at a party to answer fully (1 = almost everyone knows it, 5 = people who like the topic, 10 = only real experts).` },
    ...
    return v.filter((x) => qs.some((q) => q.index === x.index)).map((x) => ({ i: x.index, ok: !!x.ok, d: clampD(x.difficulty), problems: [String(x.problem || "it failed review").slice(0, 240)] }));
  } catch { return []; }
}
