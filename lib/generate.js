...
${SAFETY}`;
  // A failed call only costs its drafts: the spares usually cover it.
  const settled = await Promise.allSettled(chunks.map((chunk) => chat([
  ...
  if (results.every((r) => !r)) throw settled[0].reason;
  ...
  // Too few drafts: report why (usually the writer being unavailable), not a generic failure.
  if (drafts.filter(Boolean).length < n) throw settled.find((r) => r.status === "rejected")?.reason || new Error("Model returned an incomplete ladder");

  // Quality pass: the reviewer checks and rates every draft. The ladder takes the cleanest drafts spread evenly
  // from easiest to hardest; only when too few pass are fresh candidates written for the gaps and reviewed.
  ...
  const unreviewed = new Set(verdicts.filter((v) => v.unreviewed).map((v) => v.i));
  ...
  fill((x) => !bad.has(x) && !minor.has(x) && !unreviewed.has(x)); // clean drafts first
  fill((x) => minor.has(x));      // then drafts with only a minor quibble (a bit obscure, one part)
  fill((x) => unreviewed.has(x)); // only if the reviewer could not be reached for some
  ...
  if (holes.length) {
    ...
    const tier = (c) => { const v = vs.get(c.index); return !v || v.unreviewed ? 1 : v.ok ? 0 : v.minor ? 1 : 2; };
  ...
  // Labels follow the reviewer's rating when there is one, so a question is never called easier than it is.
  const labelOf = (k) => { const d = picked[k].d; return k === n - 1 || !Number.isInteger(d) ? labelFor(k, n) : d <= 2 ? "Warm-up" : d <= 4 ? "Easy" : d <= 6 ? "Medium" : d <= 8 ? "Hard" : "Expert"; };
  const questions = chosen.map((q, k) => ({ id: `q${k + 1}`, index: k, difficulty: k + 1, label: labelOf(k), prompt: q.prompt, points: q.points, rubric: q.rubric, misconceptions: q.misconceptions, ...(q.rewritten ? { rewritten: true } : {}) }));
  steps.flagged = bad.size; steps.minor = minor.size; steps.drafts = M; if (unreviewed.size) steps.unreviewed = unreviewed.size; steps.rated = picked.map((x) => x.d).join(",");
  const usage = [plan.usage, ...results.map((r) => r?.usage)].reduce((a, u) => ({ total_tokens: (a.total_tokens || 0) + (u?.total_tokens || 0) }), {});
  return { questions, model: results.find(Boolean)?.model || plan.model, ms: Date.now() - t0, usage, rewritten, steps };
}

// Reviews questions like a strict quiz editor and rates their difficulty. Returns a verdict per question.
// Never blocks generation: questions it could not review come back marked unreviewed.
...
async function review(qs, topic, apiKey, all = qs) {
  ...
  const size = Math.ceil(qs.length / Math.ceil(qs.length / REVIEW_BATCH)); const parts = [];
  for (let i = 0; i < qs.length; i += size) parts.push(qs.slice(i, i + size));
  // A failed batch is retried once; if it still fails, its questions come back marked unreviewed,
  // so the ladder only uses them when nothing reviewed is left.
  const once = (b) => reviewBatch(b, topic, apiKey, all).then((v) => v || reviewBatch(b, topic, apiKey, all));
  const out = await Promise.all(parts.map((b) => once(b).then((v) => v || b.map((q) => ({ i: q.index, ok: true, unreviewed: true, minor: false, d: null, problems: [] })))));
  return out.flat();
}
...
Also rate every question's "difficulty" from 1 to 10: how hard it is for a typical adult at a party to answer fully (1 = almost everyone knows it, 5 = people who like the topic, 10 = only real experts).` },
    ...
    const got = v.filter((x) => qs.some((q) => q.index === x.index)).map((x) => ({ i: x.index, ok: !!x.ok, minor: !x.ok && x.severity === "minor", d: clampD(x.difficulty), problems: [String(x.problem || "it failed review").slice(0, 240)] }));
    // A question the reviewer skipped counts as unreviewed, never as clean.
    return [...got, ...qs.filter((q) => !got.some((x) => x.i === q.index)).map((q) => ({ i: q.index, ok: true, unreviewed: true, minor: false, d: null, problems: [] }))];
  } catch { return null; }
}
