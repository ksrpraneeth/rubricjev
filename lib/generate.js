...
${SAFETY}`;
  ...
  // Quality pass: the grader inspects every question; flagged ones are rewritten once with the problem named.
  const flagged = await inspect(questions);
  if (flagged.length) {
    const fixes = await Promise.all(flagged.map(({ i, problems }) => chat([
      { role: "system", content: system },
      { role: "user", content: `Topic: ${topic}\nQuestions to write (1):\nQuestion ${i + 1}/${n}: ${angles[i].angle} (harder because: ${angles[i].why_harder})\nA previous draft was rejected because ${problems.join("; and ")}. Draft to avoid: "${questions[i].prompt}". Write a better one.` },
    ], "ladder_chunk", expandSchema, apiKey).then((r) => ({ i, q: r.parsed.questions[0] })).catch(() => null)));
    for (const f of fixes) if (f?.q) questions[f.i] = { ...questions[f.i], prompt: f.q.prompt, points: f.q.points.slice(0, 5), rubric: f.q.rubric.slice(0, 3).map(cleanCheck).filter(Boolean), misconceptions: f.q.misconceptions.slice(0, 2).map(cleanCheck).filter(Boolean), rewritten: true };
  }
  const usage = [plan.usage, ...results.map((r) => r.usage)].reduce((a, u) => ({ total_tokens: (a.total_tokens || 0) + (u?.total_tokens || 0) }), {});
  return { questions, model: results[0]?.model || plan.model, ms: Date.now() - t0, usage, rewritten: flagged.length };
}

// Uses the decision model as a question inspector. Never blocks generation: on any error nothing is flagged.
const INSPECT = {
  leak: { instructions: "The question text already states, or gives away, the answer to one of the things it asks for.", problem: "the question gives away its own answer" },
  vague: { instructions: "The question is vague, opinion-based, or has no single checkable factual answer.", problem: "the question is vague or opinion-based" },
  mismatch: { instructions: "At least one of the rubric checks rewards something the question never asks for.", problem: "a key point asks for something the question does not ask" },
  onepart: { instructions: "The question asks for only one thing (one word or one name), not two or three.", problem: "it only asks for one thing" },
  context: { instructions: "The question refers to something it never names (like \"that album\" or \"this game\"), so it cannot be understood on its own.", problem: "it refers to something it never names, so it does not stand alone" },
};
async function inspect(questions) {
  const key = process.env.JEV_API_KEY; if (!key) return [];
  try {
    const out = await Promise.all(questions.map(async (q, i) => {
      const body = { model: "jev-latest", state: { question: q.prompt, rubric_checks: q.rubric.map((c) => `The answer ${c}`), answer_key: q.points },
        questions: Object.fromEntries(Object.entries(INSPECT).map(([k, v]) => [k, { type: "noul", instructions: v.instructions }])) };
      const ctrl = new AbortController(); const timer = setTimeout(() => ctrl.abort(), 6000);
      const r = await fetch("https://api.typesafe.ai/v1/systemone", { method: "POST", signal: ctrl.signal, headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify(body) }).finally(() => clearTimeout(timer));
      if (!r.ok) return null;
      const a = (await r.json()).answers || {};
      const problems = Object.entries(INSPECT).filter(([k]) => (a[k]?.noul ?? 0) >= 0.75).map(([, v]) => v.problem);
      return problems.length ? { i, problems } : null;
    }));
