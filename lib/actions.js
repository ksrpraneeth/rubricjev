...
import { statsScope } from "./stats.js";
import { logJourney } from "./journey.js";

const MUTATING = new Set(["join", "settings", "answer", "next", "ready", "again", "leave", "kick", "end"]);
// opts.gpc: the browser sent Global Privacy Control. Tests, opted-out players and GPC are never counted.
export async function runAction(action, body, ip, opts = {}) {
  const test = ip === game.TEST_IP, off = test || !!body?.ns || !!opts.gpc;
  return statsScope.run({ off, test }, () => runCounted(action, body, ip));
}
async function runCounted(action, body, ip) {
...
async function run(action, body, ip) {
  ...
  switch (action) {
    ...
    case "journey": await game.limitJourney(ip); return logJourney(body);
