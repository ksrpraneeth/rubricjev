...
function memoryStore() {
  const kv = new Map(), hashes = new Map(), bus = new EventEmitter();
  ...
  return {
    ...
    async subscribe(ch, fn) { bus.on(ch, fn); return async () => bus.off(ch, fn); },
    async expire() {},
  };
}
...
function upstashStore() {
  ...
  return {
    ...
    async subscribe(ch, fn) {
      ...
      await new Promise((res) => { let done = false; const ok = () => { if (!done) { done = true; res(); } }; sub.on("subscribe", ok); setTimeout(ok, 1500); });
